...
1. Basic (context from request body):
   @app.post("/api/refunds")
   async def refunds(amount: float, _: dict = require_policy("refunds.v1", AGENT_ID)):
       return {"success": True}

2. Explicit context:
   @app.post("/api/data/export")
   async def export(rows: int, _: dict = require_policy_with_context("data_export.v1", AGENT_ID, {"rows": rows})):
       return {"success": True}
