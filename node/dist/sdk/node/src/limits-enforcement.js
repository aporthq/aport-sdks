...
exports.getAgentLimits = getAgentLimits;
exports.validateLimitsConfig = validateLimitsConfig;
const limits_1 = require("../../../functions/utils/limits");
/**
 ...
 */
function checkLimits(agent, context, config = {}) {
    ...
    const limitChecker = (0, limits_1.createLimitChecker)(limits);
    // Check each limit
    for (const [limitKey, limitValue] of Object.entries(limits)) {
        ...
        switch (limitKey) {
            ...
            case "allow_pii":
                checkResult = limitChecker.checkExportLimit(context.rows || 0, context.contains_pii || false);
                break;
 ...
 */
function isLimitExceeded(agent, limitKey, context, config = {}) {
    const finalConfig = { ...DEFAULT_CONFIG, ...config };
    if (!finalConfig.enabled) {
        return { allowed: true };
    }
    ...
    switch (limitKey) {
        ...
        case "data_export_max_rows":
        case "allow_pii":
            return limitChecker.checkExportLimit(context.rows || 0, context.contains_pii || false);
        case "max_deploys_per_day":
 ...
 */
function getAgentLimits(agent) {
    return agent?.limits || {};
}
/**
 * Validate limits configuration
 */
function validateLimitsConfig(limits) {
    const errors = [];
...
//# sourceMappingURL=limits-enforcement.js.map