...
function validateFinancialInput(context) {
    const errors = [];
    const warnings = [];
    ...
    if (sanitizedContext.currency) {
        ...
        if (sanitizedContext.currency.length !== 3 ||
            !/^[A-Z]+$/.test(sanitizedContext.currency)) {
            errors.push("currency must be exactly 3 uppercase letters");
        }
    }
    // Validate idempotency key format
    if (sanitizedContext.idempotency_key &&
        !/^[a-zA-Z0-9_-]{10,64}$/.test(sanitizedContext.idempotency_key)) {
        errors.push("idempotency_key must be 10-64 characters, alphanumeric with hyphens/underscores");
    }
    ...
    if (sanitizedContext.order_id) {
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(sanitizedContext.order_id)) {
            errors.push("order_id must contain only alphanumeric characters, hyphens, and underscores");
        }
    }
    ...
    if (sanitizedContext.customer_id) {
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(sanitizedContext.customer_id)) {
            errors.push("customer_id must contain only alphanumeric characters, hyphens, and underscores");
        }
    }
...
function generateSecureId(prefix = "ref") {
    const crypto = require("crypto");
    const timestamp = Date.now();
    const randomBytes = crypto.randomBytes(16).toString("hex");
    return `${prefix}_${timestamp}_${randomBytes}`;
}
function validateAmountPrecision(amount, currency) {
