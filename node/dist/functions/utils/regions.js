...
exports.regionValidator = exports.RegionValidator = exports.ISO_3166_SUBDIVISIONS = exports.ISO_3166_COUNTRIES = void 0;
...
exports.getValidCountryCodes = getValidCountryCodes;
exports.getValidSubdivisionCodes = getValidSubdivisionCodes;
/**
 * ISO-3166-1 Alpha-2 country codes
 * Source: https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2
 */
exports.ISO_3166_COUNTRIES = new Set([
    // A
    "AD",
    "AE",
    "AF",
 ...
 */
exports.ISO_3166_SUBDIVISIONS = new Set([
    // United States (US-XX)
    "US-AL",
    "US-AK",
    "US-AZ",
...
function validateRegion(region) {
    ...
    if (!/^[A-Z]{2}(-[A-Z0-9]{1,3})?$/.test(regionUpper)) {
        errors.push(`Invalid region format: ${region}. Must be ISO-3166 format (CC or CC-SS)`);
        return { valid: false, errors, warnings };
    }
    ...
    if (!exports.ISO_3166_COUNTRIES.has(countryCode)) {
    ...
    if (subdivisionCode) {
        ...
        if (!exports.ISO_3166_SUBDIVISIONS.has(fullCode)) {
            // Don't error on unknown subdivisions, just warn
            // This allows for less common subdivisions while still validating format
            warnings.push(`Unknown subdivision code: ${fullCode}. Verify this is a valid ISO-3166-2 code.`);
        }
    }
...
function isValidCountryCode(code) {
    if (!code || typeof code !== "string")
        return false;
    return exports.ISO_3166_COUNTRIES.has(code.toUpperCase().trim());
}
/**
...
function isValidSubdivisionCode(code) {
    ...
    if (!/^[A-Z]{2}-[A-Z0-9]{1,3}$/.test(codeUpper))
        return false;
    const [countryCode] = codeUpper.split("-");
    // Country must be valid and subdivision must be in our known list
    return (exports.ISO_3166_COUNTRIES.has(countryCode) && exports.ISO_3166_SUBDIVISIONS.has(codeUpper));
}
/**
 * Get all valid country codes
 */
function getValidCountryCodes() {
    return Array.from(exports.ISO_3166_COUNTRIES).sort();
}
/**
 * Get all valid subdivision codes for a country
 */
function getValidSubdivisionCodes(countryCode) {
    if (!isValidCountryCode(countryCode))
        return [];
    const prefix = countryCode.toUpperCase() + "-";
    return Array.from(exports.ISO_3166_SUBDIVISIONS)
...
exports.RegionValidator = RegionValidator;
/**
 * Global validator instance for performance
 */
exports.regionValidator = new RegionValidator();
