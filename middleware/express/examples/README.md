...
// All routes below require payments.refund.v1 policy
...
app.post("/api/export", 
  requirePolicy("data.export.v1"),  // No agent ID
  (req, res) => {
...
app.post("/api/repo/pr", 
  requirePolicyWithContext("repo.v1", { 
...
- **payments.refund.v1**: Payment refunds with currency and region validation
- **data.export.v1**: Data export with row limits and PII handling
- **messaging.v1**: Messaging with rate limits and channel restrictions
- **repo.v1**: Repository operations with branch protection and PR size limits
