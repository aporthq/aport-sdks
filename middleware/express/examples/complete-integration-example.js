...
app.post("/api/refunds/manual", async (req, res) => {
  ...
  const r = await fetch(
    ...
    }/api/verify/policy/payments.refund.v1`,
    {
      method: "POST",
...
app.post(
  "/api/repo/pr",
  requirePolicy("repo.v1", AGENT_ID),
  async (req, res) => {
    // Context validation automatically checks:
 ...
 * - Row limits and PII access (data.export.v1)
 * - Repository, branch, size, path, and review requirements (repo.v1)
 * - Channel access, rate limits, and mention policies (messaging.v1)
 *
