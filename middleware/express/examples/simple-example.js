...
app.use(
  agentPassportMiddleware({
    policyId: "payments.refund.v1", // Enforces refunds policy globally
    failClosed: true,
  })
);
...
app.post(
  "/api/messages/send",
  requirePolicy("messaging.v1"), // No agent ID - uses header
  (req, res) => {
...
✅ GLOBAL: Enforce one policy on all routes
   app.use(agentPassportMiddleware({ policyId: "payments.refund.v1" }));

✅ EXPLICIT: Most secure, explicit agent ID
   app.post("/api/refunds", requirePolicy("payments.refund.v1", AGENT_ID), handler);

✅ HEADER: Backward compatible, uses header
   app.post("/api/export", requirePolicy("data.export.v1"), handler);

✅ CONTEXT: Custom context for complex scenarios
   app.post("/api/pr", requirePolicyWithContext("repo.v1", context, AGENT_ID), handler);
