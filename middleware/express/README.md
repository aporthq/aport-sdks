...
Express middleware for AI Agent Passport Registry verification and policy enforcement.
...
- **Policies**: Choose from `payments.refund.v1`, `data.export.v1`, `messaging.v1`, `repo.v1`
...
app.use(agentPassportMiddleware({
  policyId: "payments.refund.v1",  // Enforces refunds policy
  failClosed: true
}));

// All routes now require payments.refund.v1 policy compliance
...
// Header fallback
app.post('/api/export', 
  requirePolicy("data.export.v1"),  // Uses X-Agent-Passport-Id header
  (req, res) => {
...
app.post('/api/data/export', 
  requirePolicy("data.export.v1", AGENT_ID),
  (req, res) => res.json({ message: "Export created" })
);

app.post('/api/messages/send', 
  requirePolicy("messaging.v1", AGENT_ID),
...
- `options.policyId` (string): Policy ID to enforce (e.g., "payments.refund.v1")
...
- `policyId` (string): Policy ID to enforce (e.g., "payments.refund.v1")
...
app.use(agentPassportMiddleware({
  policyId: "payments.refund.v1"  // Agent ID from X-Agent-Passport-Id header
}));
```
