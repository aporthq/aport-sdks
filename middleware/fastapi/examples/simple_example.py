...
app.add_middleware(
    ...
    options=AgentPassportMiddlewareOptions(
        policy_id="payments.refund.v1",  # Enforces refunds policy globally
        fail_closed=True
    )
)

# All routes below now require payments.refund.v1 policy compliance
...
# Add the policy middleware
app.middleware("http")(require_policy("messaging.v1"))  # No agent ID - uses header
...
✅ GLOBAL: Enforce one policy on all routes
   app.add_middleware(agent_passport_middleware, options=AgentPassportMiddlewareOptions(policy_id="payments.refund.v1"))
...
✅ HEADER: Backward compatible, uses header
   app.middleware("http")(require_policy("data.export.v1"))

✅ CONTEXT: Custom context for complex scenarios
   app.middleware("http")(require_policy_with_context("repo.v1", context, AGENT_ID))
