...
FastAPI middleware for AI Agent Passport Registry verification and policy enforcement.
...
app.add_middleware(
    ...
    options=AgentPassportMiddlewareOptions(
        policy_id="payments.refund.v1",  # Enforces refunds policy
        fail_closed=True
    )
)
...
# Add the policy middleware
app.middleware("http")(require_policy("data.export.v1"))  # Uses X-Agent-Passport-Id header
```
...
app.middleware("http")(require_policy("data.export.v1", AGENT_ID))
@app.post("/api/data/export")
async def export(request: Request):
    return {"message": "Export created"}

app.middleware("http")(require_policy("messaging.v1", AGENT_ID))
...
- `options.policy_id` (str): Policy ID to enforce (e.g., "payments.refund.v1")
...
- `policy_id` (str): Policy ID to enforce (e.g., "payments.refund.v1")
