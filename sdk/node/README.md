...
A production-grade thin Node.js SDK for the AI Agent Passport Registry, providing easy integration with agent authentication and policy verification via API calls. All policy logic, counters, and enforcement happen on the server side.
...
- `policyId` (string): The ID of the policy pack (e.g., `payments.refund.v1`, `release.v1`).
...
Verifies the `payments.refund.v1` policy.
...
Verifies the `repo.v1` policy.

#### Additional Policy Methods
The `PolicyVerifier` also includes convenience methods for other policies:
- `verifyRelease()` - Verifies the `release.v1` policy
- `verifyDataExport()` - Verifies the `data.export.v1` policy  
- `verifyMessaging()` - Verifies the `messaging.v1` policy
...
try {
  await client.verifyPolicy("invalid-agent", "payments.refund.v1", {});
} catch (error) {
  if (error instanceof AportError) {
...
```javascript
const decision = await client.verifyPolicy("agent-123", "payments.refund.v1", {});
