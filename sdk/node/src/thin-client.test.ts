...
describe("APortClient", () => {
  ...
  describe("verifyPolicy", () => {
    it("should make a POST request to the policy endpoint", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "https://api.aport.io/api/verify/policy/payments.refund.v1",
    ...
    it("should handle API errors with reasons", async () => {
      ...
      try {
        await client.verifyPolicy("agent-123", "payments.refund.v1", {});
  ...
  describe("getDecisionToken", () => {
    it("should make a POST request to the token endpoint", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "https://api.aport.io/api/verify/token/payments.refund.v1",
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({
            agent_id: "agent-123",
...
describe("PolicyVerifier", () => {
  ...
  describe("verifyRefund", () => {
    it("should call verifyPolicy with payments.refund.v1 policy", async () => {
      const mockResponse: PolicyVerificationResponse = {
        decision_id: "dec_123",
        allow: true,
        reasons: [],
  ...
  describe("verifyRepository", () => {
    it("should call verifyPolicy with repo.v1 policy", async () => {
      const mockResponse: PolicyVerificationResponse = {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "https://api.aport.io/api/verify/policy/repo.v1",
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({
            agent_id: "agent-123",
