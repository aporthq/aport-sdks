...
A production-grade thin Python SDK for the AI Agent Passport Registry, providing easy integration with agent authentication and policy verification via API calls. All policy logic, counters, and enforcement happen on the server side.
...
- `policy_id` (str): The ID of the policy pack (e.g., `payments.refund.v1`, `release.v1`).
...
- `verify_release()` - Verifies the `release.v1` policy
- `verify_data_export()` - Verifies the `data.export.v1` policy  
- `verify_messaging()` - Verifies the `messaging.v1` policy
...
try:
    await client.verify_policy("invalid-agent", "payments.refund.v1", {})
except AportError as error:
...
```python
decision = await client.verify_policy("agent-123", "payments.refund.v1", {})
